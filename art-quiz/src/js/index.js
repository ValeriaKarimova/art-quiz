...
const quizTemplate =
  '<div class="picture_question"><img src="{{image}}" alt="picture">{{pagination}}</div><div class="answers">{{options}}</div>';
const optionTemplate =
  '<div class="option" data-image-num={{imageNum}}><h5>{{title}}</h5></div>';
const quizRoot = document.querySelector(".quiz");
const rightOverlayTemplate =
  '<div class="answer_overlay"><div class="basis"><div><img class="icon" src="images/other/true.png" alt="img"></div><div><img class="basis_img" src="{{img}}" alt="image"></div><div class="image_description"><p>{{title}}</p><p>{{name}}</p><p>{{year}}</p></div><div><button><span>next</span></button></div></div></div>';
const falseOverlayTemplate =
  '<div class="answer_overlay"><div class="basis"><div><img class="icon" src="images/other/false.png" alt="img"></div><div><img class="basis_img" src="{{img}}" alt="image"></div><div class="image_description"><p>{{title}}</p><p>{{name}}</p><p>{{year}}</p></div><div><button><span>next</span></button></div></div></div>';
const resultTemplate =
  '<div class="result_overlay"><div class="result_basis"><div class="results_description"><p>Congratulations!</p><p>Правильных ответов: {{result}}</p></div><div><img src="images/other/finish.png" alt="img"></div><div><button class="ok_button"><span>Ok</span></button></div></div></div>';
const scorePageTemplate =
  '<div class="score_round" {{style}}><div class="true_false"><img src={{true_false}} alt="icon"></div><div class="image_score_description hide"><p>{{title}}</p><p>{{name}}</p><p>{{year}}</p></div></div>';
...
function getOptions(factsData, factIndex) {
  ...
  let randomIndex1 = getUniqueIndex(factsData, [fact.author]);
  let randomFact1 = factsData[randomIndex1];
  let randomIndex2 = getUniqueIndex(factsData, [
    fact.author,
    randomFact1.author,
  ]);
  let randomFact2 = factsData[randomIndex2];
  let randomIndex3 = getUniqueIndex(factsData, [
    fact.author,
    randomFact1.author,
    randomFact2.author,
  ]);
...
function fillQuizCategory(kindData) {
  ...
  for (let groupData of groupsData) {
    ...
    scoreStar.addEventListener("click", () =>
      fillScorePage(factsData, kindData, groupData)
    );
...
function fillScorePage(factsData, kindData, groupData) {
  ...
  for (let item of imageAnswers) {
    let selectedRound = factsData[item];
    let scoreHtml;
...
function handleVariantClick(
  ...
  if (isCorrectAnswer) {
    ...
    setTimeout(() => showRightOverlay(correctFact, groupData, kindData), 300);
    correctAnswers++;
  } else {
    selectedOptionDiv.classList.add("red");
    setTimeout(() => showFalseOverlay(correctFact, groupData, kindData), 300);
...
function showRightOverlay(correctFact, groupData, kindData) {
  let imagePath = `images/img/${correctFact.imageNum}.jpg`;
  let overlayHtml = rightOverlayTemplate
...
function showFalseOverlay(correctFact, groupData, kindData) {
  let imagePath = `images/img/${correctFact.imageNum}.jpg`;
  let overlayHtml = falseOverlayTemplate
    .replace("{{img}}", imagePath)
    .replace("{{title}}", correctFact.author)
    .replace("{{name}}", correctFact.name)
    .replace("{{year}}", correctFact.year);
...
async function fillQuizKinds() {
  ...
  for (let kindData of structureData) {
    let kindHtml = kindTemplate
      .replace("{{image}}", kindData.image)
      .replace("{{title}}", kindData.title);
